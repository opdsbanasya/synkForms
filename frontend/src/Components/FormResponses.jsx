  ...
  const fetchFormAndResponses = useCallback(async () => {
    try {
      ...
      const [formResponse, responsesResponse] = await Promise.all([
        ...
        axios.get(`${BASE_URL}/forms/${id}/responses`, { withCredentials: true })
      ]);
      ...
      console.log(err);
      alert("Error loading form responses: " + (err.response?.data?.message || err.message));
    } finally {
      setIsLoading(false);
    }
  }, [id, dispatch]);
    ...
    <div className="max-w-7xl mx-auto p-6">
        ...
        {responses.length === 0 ? (
            <div className="text-center py-12 bg-gray-50 rounded-lg">
                ...
                <h2 className="text-xl font-semibold text-gray-600 mb-2">No responses yet</h2>
                                                ...
                                                {responseField ? (
                                                    Array.isArray(responseField.value) ? (
                                                        responseField.value.length > 0 ? (
                                                            <div className="flex flex-wrap gap-1">
                                                                {responseField.value.map((val, valIndex) => (
                                                                    <span key={valIndex} className="bg-blue-100 text-blue-800 px-2 py-1 rounded text-xs">
                                                                        {val}
                                                                    </span>
                                                                ))}
                                                            </div>
                                                        ...
                                                        <div className="max-w-xs">
                                                            <p className="whitespace-pre-wrap break-words">{responseField.value}</p>
                                                        </div>
                                                    ) : (
